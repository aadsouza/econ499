...
cap log close
log using $linreg/stagdid-fe-sltt.log, replace
...
esttab wagstagdid_mal_all wagstagdid_mal_all_unemp wagstagdid_mal_all_sltt wagstagdid_mal_all_fe_sltt wagstagdid_fem_all wagstagdid_fem_all_unemp wagstagdid_fem_all_sltt wagstagdid_fem_all_fe_sltt using $tabs/feslttwagstagdid1.tex, se title(Difference in Differences of Real Log Wages on Right to Work Laws Treatment in State and Time) mgroups("Men" "Women", pattern(1 0 0 0 1 0 0 0) prefix(\multicolumn{@span}{c}{) suffix(}) span erepeat(\cmidrule(lr){@span})) keep(blackrelwhite treat_st treat_stblackrelwhite) nomtitles replace label starlevels(* 0.1 ** 0.05 *** 0.01) 

esttab nwagstagdid_mal_all nwagstagdid_mal_all_unemp nwagstagdid_mal_all_sltt nwagstagdid_mal_all_fe_sltt nwagstagdid_fem_all nwagstagdid_fem_all_unemp nwagstagdid_fem_all_sltt nwagstagdid_fem_all_fe_sltt using $tabs/feslttwagstagdid2.tex, se title(Difference in Differences of Real Log Wages on Right to Work Laws Treatment in State and Time) mgroups("Men" "Women", pattern(1 0 0 0 1 0 0 0) prefix(\multicolumn{@span}{c}{) suffix(}) span erepeat(\cmidrule(lr){@span})) keep(blackrelwhite treat_st treat_stblackrelwhite) nomtitles replace label starlevels(* 0.1 ** 0.05 *** 0.01)

esttab uwagstagdid_mal_all uwagstagdid_mal_all_unemp uwagstagdid_mal_all_sltt uwagstagdid_mal_all_fe_sltt uwagstagdid_fem_all uwagstagdid_fem_all_unemp uwagstagdid_fem_all_sltt uwagstagdid_fem_all_fe_sltt using $tabs/feslttwagstagdid3.tex, se title(Difference in Differences of Real Log Wages on Right to Work Laws Treatment in State and Time) mgroups("Men" "Women", pattern(1 0 0 0 1 0 0 0) prefix(\multicolumn{@span}{c}{) suffix(}) span erepeat(\cmidrule(lr){@span})) keep(blackrelwhite treat_st treat_stblackrelwhite) nomtitles replace label starlevels(* 0.1 ** 0.05 *** 0.01) scalars("unemp_level State Monthly Unemployment Rate" "yr_dum Year Fixed Effect" "sltt State Linear Time Trends")

panelcombine, use($tabs/feslttwagstagdid1.tex $tabs/feslttwagstagdid2.tex $tabs/feslttwagstagdid3.tex) paneltitles("All (Black and White) People" "People Not Covered by Union - Spillover Effect" "People Covered by Union - Direct Effect") columncount(3) save($tabs/fin_feslttwagstagdid.tex)
