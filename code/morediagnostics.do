** In this dofile we try more diagnostics for the RIF-DiD analysis of RTW laws
...
cap log close
log using $linreg/morediagnostics.log, replace
...
sum lwage3 if female == 0, detail

scalar mquantwage50 = r(p50)

gen mindic_greatermed =.
	replace mindic_greatermed = 1 if lwage3 >  mquantwage50 & female == 0
	replace mindic_greatermed = 0 if lwage3 <= mquantwage50 & female == 0
       
gen blackmindic_greatermed = blackrelwhite*mindic_greatermed

gen treat_stmindic_greatermed = treat_st*mindic_greatermed
 
gen treat_stblackmindic_greatermed = treat_stblackrelwhite*mindic_greatermed

sum lwage3 if female == 1, detail

scalar fquantwage50 = r(p50)

gen findic_greatermed =.
	replace findic_greatermed = 1 if lwage3 >  fquantwage50 & female == 1
	replace findic_greatermed = 0 if lwage3 <= fquantwage50 & female == 1 
       
gen blackfindic_greatermed = blackrelwhite*findic_greatermed

gen treat_stfindic_greatermed = treat_st*findic_greatermed

gen treat_stblackfindic_greatermed = treat_stblackrelwhite*findic_greatermed

reg lwage3 blackrelwhite treat_st mindic_greatermed treat_stblackrelwhite blackmindic_greatermed treat_stmindic_greatermed treat_stblackmindic_greatermed i.state i.year i.nind educ exper exper2 exper3 exper4 edex i.ee_cl marr partt  public cmsa i.nocc3 unemp i.quarter [aw = finalwt1] if female == 0, vce(cluster state) 

reg lwage3 blackrelwhite treat_st findic_greatermed treat_stblackrelwhite blackfindic_greatermed treat_stfindic_greatermed treat_stblackfindic_greatermed i.state i.year i.nind educ exper exper2 exper3 exper4 edex i.ee_cl marr partt  public cmsa i.nocc3 unemp i.quarter [aw = finalwt1] if female == 1, vce(cluster state) 
